...
import aw2m.Property;
import aw2m.PropertyInstance;
import aw2m.Terrain;
import aw2m.Unit;
...
public class MapLoader {
    ...
    public static void assignTerrain(byte terrainType, byte x, byte y) {
        ...
        if (MapLoader.map[x][y] != null) {
            switch (terrainType) {
                case Terrain.HQ:
                    MapLoader.map[x][y].terrain = Terrain.getHq();
                    MapLoader.map[x][y].isProperty = true;
                    break;
                case Terrain.CITY:
                    MapLoader.map[x][y].terrain = Terrain.getCity();
                    MapLoader.map[x][y].isProperty = true;
                    break;
                case Terrain.BASE:
                    MapLoader.map[x][y].terrain = Terrain.getBase();
                    MapLoader.map[x][y].isProperty = true;
                    break;
                case Terrain.AIRPORT:
                    MapLoader.map[x][y].terrain = Terrain.getAirport();
                    MapLoader.map[x][y].isProperty = true;
                    break;
                case Terrain.PORT:
                    MapLoader.map[x][y].terrain = Terrain.getPort();
                    MapLoader.map[x][y].isProperty = true;
                    break;
                case Terrain.BRIDGE:
    ...
    static void assignPlayerToProperty(byte playerID, byte propertyType, byte x, byte y) {
        ...
        if (MapLoader.map[x][y] != null) {
            if (MapLoader.map[x][y].isProperty) {
                ...
                MapLoader.map[x][y].property = Property.getAbstractProperty(propertyType);
                MapLoader.map[x][y].property.propertyType = propertyType;
                MapLoader.map[x][y].propertyInstance = new PropertyInstance(Game.players[playerID]);
            }
            else {
                //DEBUG
