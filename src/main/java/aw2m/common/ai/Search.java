...
public class Search {
    ...
    public Branch createBranchFromOptimalNodes(LinkedList<Unit> playerUnits) {
        ...
        // List simulating the branch formed by optimal nodes of each attacking
        // unit
        ...
        for (Unit clonedAttacker : clonedAttackingUnits) {
            ...
            // Store them in a list. This will be the set of clones the whole
            // branch will work with
            ...
            // To create such enemy clone list: we must iterate through the
            // entire map,
            ...
            // Receiving as key a pair of coordinates: their coordinates on the
            // game map
            ...
            for (GridCell gridRow[] : map) {
                for (GridCell gridCell : gridRow) {
                    if (gridCell.unit != null) {
                        if (gridCell.unit.player != null && clonedAttacker.player != null) {
            ...
            // Keep a reference to the optimal value found so far, and during
            // search
            Node optimalNode = null;

            // First, get deathZone
            LinkedHashSet<GridCell> deathZone = Logic.getDeathZone(map,
                                                                   clonedAttacker);
            // For each enemy on deathZone
            for (GridCell cell : deathZone) {
                if (cell.unit != null) {

                    // Get the equilvalent unit from the map of clones for this
                    // branch
                    ...
                    if (clonedDefender != null) {
                        ...
                        if (clonedDefender.player.team != clonedAttacker.player.team) {
                            ...
                            if (Unit.getEngagingWeapon(clonedAttacker,
                                                       clonedDefender) != Unit.CANT_ENGAGE) {
                                // Eval node to see if it becomes part of the
                                // branch
                                // Use M-Coefficient
                                short[] combatOutcome = Logic.combatSimulation(
                                        clonedAttacker, clonedDefender);
                                short nodeEvalValue = NodeEvalFunctions
                                        .M_balanceCoefficient(clonedAttacker,
                                                              clonedDefender, combatOutcome);
                                this.nodesGenerated++;

                                if (optimalNode == null) {
                                    // No other optimal candidate exists
                                    // Assign current value to optimal node
                                    //System.out.println("Assigning a new value ot Optimal node");
                                    optimalNode = new Node(clonedAttacker,
                                                           clonedDefender, nodeEvalValue);
                                }
                                else {
                                    // Compare optimal vs new
                                    if (optimalNode.value < nodeEvalValue) {
                                        // Optimal is no longer THE optimal
                                        // value
            ...
            if (optimalNode != null) {
                ...
                // And so, being able to progress the branch evolution as a
                // natural process.
    ...
    public void createAllPossibleBranches(LinkedList<Unit> attackingUnits) {
        ...
        // This for is supposed to give all permutations possible for all
        // starting
        ...
        if (attackingUnits.size() > 1) {
            for (Unit u : attackingUnits) {
                ...
                if (index + 1 < attackingUnits.size()) {
                    // Get an iterator
                    ListIterator<Unit> iterator = attackingUnits
